...
def output_projection_layer(num_units,
    ...
    def sampled_sequence_loss(outputs, targets, masks, keyword_tensor, word_type):
        with variable_scope.variable_scope('decoder/%s' % name):
            ...
            out_logits_labels = tf.expand_dims(tf.reduce_sum(
                result_before_softmax * tf.one_hot(tf.reduce_sum(local_labels, axis = 1), num_symbols), axis = 1),
                axis = 1)
            out_logits = tf.concat([out_logits_labels, out_logits_sampled], 1)
            out_labels = tf.concat([tf.ones_like(out_logits_labels, tf.float32),
                                    tf.zeros_like(out_logits_sampled, tf.float32)], 1)
