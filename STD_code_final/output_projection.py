...
def output_projection_layer(num_units,
                            num_symbols,
                            num_qwords,
                            num_samples=None,
                            question_data=True,
    ...
    def output_fn(outputs, keyword_tensor):
        with variable_scope.variable_scope('%s' % name):
            ...
            result_p_softmax = tf.Print(result_p_softmax,
                                        ["result_p_softmax", result_p_softmax,
                                         "result_p_sum", tf.reduce_sum(result_p_softmax, axis = 1)],
                                        summarize=6)
            print("result_p_softmax.shape:", result_p_softmax.get_shape())

            # batch * 3 * num_symbols
            result_p_expand = tf.matmul(tf.expand_dims(result_p_softmax, axis=2),
                                        tf.ones([tf.shape(result_p_softmax)[0], 1, num_symbols],tf.float32))
            ...
            result_final = tf.Print(result_final,
                                    ["result_final", result_final,
                                     "result_final_sum", tf.reduce_sum(result_final, axis=1)])
    ...
    def sampled_sequence_loss(outputs, targets, masks, keyword_tensor, word_type):
        with variable_scope.variable_scope('decoder/%s' % name):
            ...
            local_labels = tf.Print(local_labels,
                                    ["local_labels", local_labels,
                                     "tf.reduce_sum(local_labels)", tf.reduce_sum(local_labels)])
            ...
            result_p_labels = tf.Print(result_p_labels,
                                       ["result_p_labels", result_p_labels],
                                       summarize = 6)
            ...
            # (batch * len) * 3 * num_symbols
            result_p_expand = tf.matmul(tf.expand_dims(result_p_labels, axis=2),
                                        tf.ones([tf.shape(result_p_softmax)[0], 1, num_symbols],tf.float32))
            print("result_p_expand.shape:", result_p_expand.get_shape())
