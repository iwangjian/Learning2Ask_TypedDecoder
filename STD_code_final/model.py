...
class Seq2SeqModel(object):
    def __init__(self,
        ...
        self.decoder_mask = tf.reshape(
            tf.cumsum(tf.one_hot(self.responses_length-1,
        ...
        if use_bidrnn:
            ...
            encoder_output, encoder_state = multi_bidirectional_rnn(
                encoder_cell, num_units / 2, num_layers, self.encoder_input, self.posts_length)
        else:
            # rnn encoder
            encoder_output, encoder_state = dynamic_rnn(
                cell, self.encoder_input, self.posts_length, dtype=tf.float32, scope="encoder")
